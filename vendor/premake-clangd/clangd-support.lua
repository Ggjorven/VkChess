...
p.modules.clangd_support = {}
local m = p.modules.clangd_support
...
function m.getCommonFlags(cfg)
	...
	flags = table.join(flags, cfg.cppdialect) -- ggjorven
...
function m.getConfig(prj)
	if _OPTIONS["clangd-support-config"] then
		return project.getconfig(prj, _OPTIONS["clangd-support-config"], _OPTIONS["clangd-support-platform"])
	end
